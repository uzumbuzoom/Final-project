...
ggplot(df,aes(x=year,y=life.exp.by.65))+
  geom_line(aes(y=life.exp.by.65), color = "blue", size=2) + 
  geom_line(aes(y=life.exp.by.birth), color = "pink", size=2) +
  theme(plot.title = element_text(hjust = 0.5))+
...
The average life expectancy between `r params$start_year ` and `r params$end_year ` for people by 65 years old in `r params$start_year ` was `r round((df$life.exp.by.65[df$year >= params$start_year & df$year <= params$end_year]))`
...
The average life expectancy between `r params$start_year ` and `r params$end_year ` for people by birth in `r params$start_year ` was `r round((df$life.exp.by.birth[df$year >= params$start_year & df$year <= params$end_year]))`
