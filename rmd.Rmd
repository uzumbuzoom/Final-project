...
ggplot(df,aes(x=year))+
  geom_line(aes(y=unemployment.rate, color = "unemployment.rate"), size=2) + 
  geom_line(aes(y=long.term.unemployment, color = "long.term.unemployment"), size=2) +
...
The average Unemployment rate between `r params$start_year ` and `r params$end_year `  in `r params$country ` was `r mean((df$life.exp.by.65[df$year >= params$start_year & df$year <= params$end_year][1]))`
